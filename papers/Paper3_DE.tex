...
\textbf{Einheiten und Notation.} Im gesamten $f(R)$-Sektor arbeiten wir in reduzierten Planck-Einheiten ($8\pi G = 1$), in denen die dimensionsbehaftete $R^2$-Kopplung der Wirkung~\eqref{eq:full_action} durch $\epsilon \equiv 16\pi G\gamma = 2\gamma$ gegeben ist. Mit dieser Konvention gilt $f(R) = R + \epsilon R^2 = R + 2\gamma R^2$ und $f_{RR} = 2\epsilon = 4\gamma$, was die Koeffizienten der Spurgleichung~\eqref{eq:trace_equation} ($6\epsilon = 12\gamma$) und der Skalaron-Masse festlegt. Explizite Faktoren von $G$ werden \"uberall dort wiederhergestellt, wo dimensionsbehaftete Zahlenwerte angegeben werden.

\textbf{Konforme \"Aquivalenz.} Der Gravitationssektor $f(R) = R + 2\gamma R^2$ ist konform \"aquivalent zur Einstein-Gravitation plus einem massiven Skalarfeld (dem \textit{Skalaron}) $\chi$:
\begin{equation}
...
\end{equation}
Da $\mathcal{F} \in [0,1]$ monoton und beschr\"ankt ist, bleibt die Masse zu allen Zeiten reell und positiv. Zu fr\"uhen Zeiten ($\mathcal{F} \to 0$) gilt $m_{\mathrm{eff}} \to \infty$ und das Skalaron ist ausgefroren. Zu sp\"aten Zeiten ($\mathcal{F} \to 1$) gilt $m_{\mathrm{eff}} = m_s$, in \"Ubereinstimmung mit $m_s^2 = 1/(6\epsilon) = 1/(12\gamma)$ aus der obigen Proposition.

\textbf{Newtonscher Grenzfall.} Im Schwachfeldlimes erzeugt das Skalaron eine Yukawa-Korrektur:
\begin{equation}
V(r) = -\frac{GM}{r}\left(1 + \frac{1}{3}\,e^{-m_{\mathrm{eff}}\,r}\right)
\label{eq:yukawa}
\end{equation}
Sonnensystem-Beschr\"ankungen erfordern $m_{\mathrm{eff}}\,r_{\mathrm{AU}} \gg 1$.

\textbf{Lokale Gravitationstests: keine Abschirmung im vorliegenden Sektor.} Fr\"uhere Fassungen dieses Papers beanspruchten eine Abschirmung des Skalarons im Sonnensystem durch einen Cham\"aleon-Mechanismus \cite{Khoury2004}, mit $m_{\mathrm{eff}} \propto \sqrt{\rho_{\mathrm{local}}/\rho_{\mathrm{cosm}}}$ und $\lambda_C^{\mathrm{solar}} \sim 20\,\mathrm{m}$. Dieser Anspruch wird zur\"uckgenommen. Er beruhte auf einem \"uberz\"ahligen Kr\"ummungsfaktor in der Skalaron-Masse; mit $m_s^2 = 1/(6\epsilon) = 1/(12\gamma)$ existiert keine solche Dichteabh\"angigkeit, denn f\"ur ein rein quadratisches $f(R)$ ist $f_{RR} = 2\epsilon$ konstant und $m_s$ damit kr\"ummungs- und folglich dichteunabh\"angig.

Das ist strukturell und nicht zuf\"allig: Cham\"aleon-Abschirmung in $f(R)$-Modellen Dunkler Energie verlangt ein mit der Kr\"ummung \textit{fallendes} $f_{RR}$ -- in der Hu-Sawicki-Klasse $f_{RR} \propto R^{-(n+2)}$ \cite{HuSawicki2007} --, was ein rein quadratischer Term nicht liefern kann. Auch die Spurkopplung ersetzt ihn nicht, sondern weist in die entgegengesetzte Richtung: $\mathcal{F} = |T|/(|T| + \rho_{\mathrm{rad}}) \to 1$ in materiedominierten, dichten Umgebungen, sodass in der Sonne $m_{\mathrm{eff}} \to m_s$ gilt, statt anzuwachsen. Die Aufgabe von $\mathcal{F}$ ist die Unterdr\"uckung des Skalarons in der \textit{Strahlungs\"ara} (BBN-Schutz), nicht in \textit{dichten lokalen} Umgebungen.

Die Vertr\"aglichkeit mit lokalen Gravitationstests -- Lunar-Laser-Ranging ($\Delta G/G < 10^{-13}$) \cite{Williams2004} und die Cassini-Schranke f\"ur den parametrisierten post-Newtonschen Parameter $\gamma_{\mathrm{PPN}} - 1 = (2{,}1 \pm 2{,}3) \times 10^{-5}$ \cite{Bertotti2003} -- ist f\"ur die Wirkung~\eqref{eq:full_action} in ihrer vorliegenden Form somit \textit{nicht} gezeigt; wir erheben diesen Anspruch nicht. Ein tragf\"ahiger Abschirmmechanismus erforderte einen zus\"atzlichen Term vom Hu-Sawicki-Typ in $f(R)$, dessen Konsequenzen f\"ur den kosmologischen Sektor Gegenstand einer eigenen Untersuchung sind.
...
\end{equation}
wobei $\chi = f_R - 1 = 4\gamma\mathcal{F}(a) R$ das Skalaronfeld ist und $m_{\mathrm{eff}}^2(a) = 1/(12\gamma\mathcal{F}(a))$. Die effektive geometrische Energiedichte des Skalarons ist $\Omega_{R^2}(a) \propto \chi(a) \cdot R(a)$, und der effektive Skalierungsexponent folgt als:
\begin{equation}
...
\begin{enumerate}
\item \textbf{Schleifen-Quantengravitation} \cite{Rovelli2004,Thiemann2007}: Die modifizierte LQC-Friedmann-Gleichung $H^2 \propto \rho(1 - \rho/\rho_c)$ hat dieselbe $dX/dt \propto (1-X^2)$-Struktur wie die S\"attigungs-ODE. Die beschr\"ankten Kr\"ummungsinvarianten aus Holonomie-Korrekturen erzeugen nat\"urlicherweise S\"attigung.

\item \textbf{Finsler-Geometrie} \cite{Bao2000,Girelli2007}: Richtungsabh\"angige Metriken $F(x, \dot{x})$ erzeugen skalenabh\"angige Gravitationseffekte (die MOND nachahmen) und nichtstandardm\"a\ss ige kosmologische Skalierung, wobei der $a^{-\beta}$-Term potentiell aus der oskulierenden Riemannschen Kr\"ummung hervorgeht.

\item \textbf{Informationstheoretische Raumzeit} \cite{Wheeler1990,Bousso2002}: Die S\"attigungs-ODE ist die logistische Wachstumsgleichung f\"ur Informationsverarbeitung, mit $\Phi_0$ als holographischer Kapazit\"atsgrenze. Die S\"attigung der Verschr\"ankungsentropie \cite{VanRaamsdonk2010} liefert den mikroskopischen Mechanismus.
...
\textbf{Superposition aller Geometrien:} Quantenmechanisch ist der Nullraum das Pfadintegral \cite{Feynman1948} \"uber alle m\"oglichen Raumzeitkonfigurationen, gleichm\"a\ss ig gewichtet. Er ist der Zustand maximaler Unsicherheit \"uber die Geometrie -- nicht "`leerer Raum"', sondern "`\"uberhaupt kein Raum"'.
...
\end{align}
Die Posterior erf\"ullt $P(\alpha_{M,0} > 0) = 99{,}99\%$. Dieser Wert darf nicht als Beleg f\"ur das \textit{Vorzeichen} von $\alpha_{M,0}$ gelesen werden: Der flache Prior ist auf $\alpha_{M,0} \in [0;\,0{,}003]$ eingeschr\"ankt, negative Werte waren also konstruktionsbedingt ausgeschlossen, und die Wahrscheinlichkeit gibt den Prior wieder, statt ihn zu pr\"ufen. Wie stark die Daten eine von null verschiedene Modifikation bevorzugen, bemisst sich an $\Delta\chi^2$ und nicht an dieser Zahl. Die kosmologischen Standardparameter ($\omega_{\mathrm{cdm}}$, $A_s$, $n_s$) sind im Wesentlichen unkorreliert mit den MG-Parametern ($|\rho| < 0{,}09$), was best\"atigt, dass die cfm\_fR-Erweiterung eine saubere, perturbative Erg\"anzung zu $\Lambda$CDM ist. Die starke Anti-Korrelation zwischen $\alpha_{M,0}$ und $n$ ($\rho = -0{,}61$) spiegelt die erwartete Entartung wider: Beide Parameter steuern die effektive Amplitude der modifizierten Gravitation.
...
Es ist auch aufschlussreich, das cfm\_fR-Modell mit dem weit verbreiteten Hu-Sawicki-$f(R)$-Modell \cite{HuSawicki2007} zu vergleichen, das entwickelt wurde, um Sonnensystem-Constraints durch einen Cham\"aleon-Mechanismus zu erf\"ullen und gleichzeitig Sp\"atzeit-Beschleunigung zu erzeugen. Das Hu-Sawicki-Modell wird durch $|f_{R0}|$, den heutigen Wert des Skalaronfeldes, parametrisiert. Planck-Constraints geben $\log_{10}|f_{R0}| < -4{,}79$ (95\% CL) \cite{PlanckMG2016}. Der CRM-Lagrange-Dichte $R + \gamma R^2$ fehlt die Cham\"aleon-Screening-Eigenschaft dieses Modells (Abschnitt~\ref{subsec:ghost_analysis}); sie unterscheidet sich ferner darin, dass die $R^2$-Modifikation \textit{universell} ist (nicht skalenabh\"angig per Konstruktion), und der Lauf wird durch die Skalaron-Dynamik kontrolliert statt durch eine konstruierte Funktionalform.
...
\item \textbf{Geisterfreiheit und Newtonscher Grenzfall:} Die Wirkung~\eqref{eq:full_action} ist nachweislich geisterfrei (Abschnitt~\ref{subsec:ghost_analysis}): $f_{RR} > 0$ schlie\ss t den Ostrogradsky-Geist aus, $m_s^2 = 1/(12\gamma) > 0$ schlie\ss t tachyonische Instabilit\"aten aus. Eine Abschirmung im Sonnensystem ist dagegen \textit{nicht} gezeigt: F\"ur ein rein quadratisches $f(R)$ ist $f_{RR}$ konstant, die Skalaron-Masse also dichteunabh\"angig, und es wirkt kein Cham\"aleon-Mechanismus. Die Vertr\"aglichkeit mit lokalen Gravitationstests bleibt damit offen (Abschnitt~\ref{subsec:ghost_analysis}).

\item \textbf{Lagrange-Herleitung von $\beta_{\mathrm{eff}}(a)$:} Die laufende Kopplung geht aus der Skalaron-Bewegungsgleichung~\eqref{eq:scalaron_eom} mit zeitabh\"angiger Masse $m_{\mathrm{eff}}^2(a) = 1/(12\gamma\mathcal{F}(a))$ hervor (Abschnitt~\ref{subsec:beta_derivation}). Die ph\"anomenologische sigmoidale Parametrisierung approximiert die numerische L\"osung. \textit{Hinweis:} Die laufende $\mu(a)$ aus Paper~II bleibt eine ph\"anomenologische \"Ubergangsfunktion; es wird keine Lagrange-Herleitung von $\mu(a)$ beansprucht.
