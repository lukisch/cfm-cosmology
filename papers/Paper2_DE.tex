...
Eine naive Kombination st\"oßt jedoch auf eine fundamentale Spannung: Das Standard-CRM fittet $\Omega_m \approx 0{,}36$, was erhebliche Dunkle Materie impliziert ($\Omega_m - \Omega_b \approx 0{,}31$). Wenn MOND korrekt ist und Dunkle Materie nicht existiert, muss das Modell allein mit $\Omega_m = \Omega_b \approx 0{,}05$ funktionieren.
...
\item \textbf{MOND:} Verst\"arkte Gravitationsanziehung bei niedrigen Beschleunigungen f\"uhrt zu schnellerem gravitativen Kollaps auf großen Skalen \cite{Asencio2023}.
\end{itemize}
...
F\"ur die Erweiterung auf ein Universum mit ausschließlich baryonischem Materieinhalt zerlegen wir das geometrische Potential in zwei Komponenten:
\begin{equation}
\begin{split}
...
\end{equation}
Beachte, dass bei $a=1$ der erste Term in Gl.~\eqref{eq:extended_cfm} $\mu_{\mathrm{eff}}\,\Omega_b$ ist, nicht $\Omega_b$ allein, da der MOND-Verst\"arkungsfaktor $\mu_{\mathrm{eff}}$ auf die baryonische Dichte zu allen Epochen einschließlich heute angewendet wird.
...
\textbf{Lagrangian-Herkunft aus $f(R) = R + 2\gamma R^2$:} Paper~III \cite{Geiger2026c} zeigt, dass der Spur-Kopplungs-Unterdr\"uckungsfaktor $\mathcal{S}(a)$ \textit{kein} ad~hoc-Postulat ist, sondern rigoros aus dem $R^2$-Sektor des CRM-Lagrangian folgt. Damit steht die Konstruktion in derselben $f(R)$-Gravitationslandschaft wie Hu--Sawicki-Screeningmodelle \cite{HuSawicki2007}, beh\"alt aber die CRM-spezifische S\"attigungsmotivation; die Verwandtschaft betrifft allein das Umfeld, denn der rein quadratische CRM-Term reproduziert die Cham\"aleon-Abschirmung jener Klasse nicht \cite{Geiger2026c}. Die Spur der Feldgleichungen f\"ur $f(R) = R + 2\gamma R^2$ ergibt:
\begin{equation}
...
Zur besseren Nachvollziehbarkeit trennt Tabelle~\ref{tab:evidenz_gate_ledger} die Evidenzebenen von den offenen Prüfgates. Dadurch wird der SN-Befund nicht als abgeschlossene Vollkosmologie gelesen.

\begin{table*}[t]
\centering
\caption{Evidenz- und Prüfgate-Übersicht für die zentralen Datenschichten in Paper II.}
\label{tab:evidenz_gate_ledger}
\footnotesize
\begin{tabular}{@{}llll@{}}
\toprule
\parbox[t]{0.17\textwidth}{\raggedright \textbf{Ebene}} & \parbox[t]{0.21\textwidth}{\raggedright \textbf{Datenobjekt}} & \parbox[t]{0.26\textwidth}{\raggedright \textbf{Aktuelle Darstellung}} & \parbox[t]{0.24\textwidth}{\raggedright \textbf{Offenes Prüfgate}} \\
\midrule
\parbox[t]{0.17\textwidth}{\raggedright SN-Hintergrundfit} & \parbox[t]{0.21\textwidth}{\raggedright 1.590 Pantheon+-SNe Ia mit $z>0{,}01$} & \parbox[t]{0.26\textwidth}{\raggedright Modellvergleich und MCMC-Posterior stützen einen krümmungsartigen Term mit $\Delta\chi^2=-26{,}3$ gegenüber $\Lambda$CDM.} & \parbox[t]{0.24\textwidth}{\raggedright Mit alternativen SN-Kovarianzen und unabhängigen Low-$z$-Systematik-Schnitten wiederholen.} \\
\parbox[t]{0.17\textwidth}{\raggedright Generalisierungstest} & \parbox[t]{0.21\textwidth}{\raggedright 5-Fold-Kreuzvalidierung auf Pantheon+} & \parbox[t]{0.26\textwidth}{\raggedright Der prädiktive $\chi^2/n$ auf Testdaten liegt für Erweitertes CRM+MOND leicht unter $\Lambda$CDM.} & \parbox[t]{0.24\textwidth}{\raggedright Folds, Seeds und Störparameterbehandlung stress-testen, bevor der Gewinn als robuste Generalisierung gilt.} \\
\parbox[t]{0.17\textwidth}{\raggedright Komprimierter gemeinsamer Hintergrund} & \parbox[t]{0.21\textwidth}{\raggedright Pantheon+ plus Planck-Distanzprioren und 9 BAO-Punkte} & \parbox[t]{0.26\textwidth}{\raggedright Die Variante mit laufender Kopplung erreicht null Frühe Dunkle Energie und einen konkurrenzfähigen gemeinsamen Hintergrundfit.} & \parbox[t]{0.24\textwidth}{\raggedright Komprimierte Priors durch vollständige Boltzmann- und CMB-Likelihood-Analyse ersetzen.} \\
\parbox[t]{0.17\textwidth}{\raggedright Laufende Kopplung und MOND-Brücke} & \parbox[t]{0.21\textwidth}{\raggedright Optimierte $\beta_{\mathrm{eff}}(a)$- und $\mu(a)$-Profile} & \parbox[t]{0.26\textwidth}{\raggedright Nachfolgende Tabellen zeigen, dass Skalenabhängigkeit das CMB-Schallhorizont-Problem konstanter $\beta$ entfernen kann.} & \parbox[t]{0.24\textwidth}{\raggedright Profile aus einer kovarianten Wirkung herleiten und Stabilität über die Rotverschiebung testen.} \\
\parbox[t]{0.17\textwidth}{\raggedright Perturbations- und Lensing-Sektor} & \parbox[t]{0.21\textwidth}{\raggedright Boltzmann-Code-Pfad, Lensing- und Bullet-Cluster-Logik} & \parbox[t]{0.26\textwidth}{\raggedright Als Validierungsprogramm dargestellt, nicht als abgeschlossener Datensatztest.} & \parbox[t]{0.24\textwidth}{\raggedright CRM-Perturbationsgleichungen angeben und gegen CMB-Spektren, Wachstum und Lensing-Surveys testen.} \\
\bottomrule
\end{tabular}
\end{table*}
...
\item \textbf{Erweitertes CRM gelingt spektakul\"ar:} Die Hinzuf\"ugung des geometrischen DM-Terms $\alpha \cdot a^{-\beta}$ stellt die Fit-Qualit\"at wieder her und \textit{\"ubertrifft} sie, mit $\Delta\chi^2 = -26{,}3$ gegen\"uber $\Lambda$CDM -- besser als sowohl $\Lambda$CDM \textit{als auch} das Standard-CRM mit großem Abstand.
...
\begin{itemize}
\item \textbf{Dieses Paper (Paper~II):} Demonstriert, \emph{dass} die MOND-Hintergrundkopplung gemeinsame SN+CMB+BAO-Kompatibilit\"at erreicht, und bestimmt die effektive Größe $\mu_{\mathrm{eff}} \approx \sqrt{\pi}$.
...
\item Das CMB-Leistungsspektrum fittet (einschließlich des dritten akustischen Maximums)
\item Das Materiedichtespektrum fittet
\end{enumerate}

AeST erreicht dies durch ein Skalarfeld $\phi$ und ein zeitartiges Vektorfeld $A_\mu$, die einen effektiven Energie-Impuls-Tensor erzeugen. Die kosmologischen Hintergrundgleichungen in AeST enthalten Terme, die mit nicht-standardm\"aßiger Skalierung zu $H^2(a)$ beitragen. Ein detaillierter Vergleich zwischen den AeST-Hintergrundgleichungen und der erweiterten CRM-Friedmann-Gleichung~\eqref{eq:extended_cfm} ist ein zentrales Ziel f\"ur zuk\"unftige Arbeiten.
...
\textit{Existenzbeweis durch AeST:} Die relativistische MOND-Theorie AeST \cite{Skordis2021} hat gezeigt, dass ein Universum mit ausschließlich baryonischem Materieinhalt ($\Omega_m = \Omega_b$) mit zus\"atzlichen geometrischen Freiheitsgraden (Skalar- + Vektorfelder) das \textit{vollst\"andige} CMB-Leistungsspektrum fitten kann, einschließlich des dritten akustischen Maximums. Das erweiterte CRM teilt die wesentlichen Zutaten mit AeST:
\begin{itemize}
...
\begin{enumerate}
\item Die geometrische "`Dunkle Materie"' ist automatisch \textit{stoßfrei} (es ist die Raumzeitkr\"ummung selbst)
...
\item \textbf{BAO-Messungen:} Baryonische akustische Oszillationen bei $z \sim 0{,}5$--$2{,}5$ (BOSS DR12 \cite{Alam2017} und DESI DR2) liefern ein unabh\"angiges Entfernungsmaß, das mit dem erweiterten CRM konsistent sein muss.
...
Wir haben gezeigt, dass ein Universum mit ausschließlich baryonischem Materieinhalt ($\Omega_m = \Omega_b \approx 0{,}05$), wobei die gravitative Rolle der Dunklen Materie vom geometrischen $R^2$-Scalaron \"ubernommen wird, kosmologische Daten \textit{\"uber alle drei großen Sonden hinweg} -- Supernovae, CMB und BAO -- kompetitiv mit und teilweise besser als $\Lambda$CDM fitten kann, bei \textit{gleicher Parameterzahl} und \textit{ohne} Early Dark Energy.
