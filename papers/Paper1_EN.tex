...
\begin{abstract}
We develop a game-theoretic framework for cosmology in which spacetime evolution is modeled as a Nash equilibrium between a metastable quantum vacuum (null space) and a spacetime bubble. The central result is the \textit{Curvature Relaxation Model} (CRM), which explains accelerated expansion not through dark energy but through a diminishing curvature return potential $\Omega_\Phi(a) = \Phi_0 \cdot \tanh(k(a - a_{\mathrm{trans}}))$ -- a geometric ``memory'' of the Big Bang energy concentration. Tested against 1,590 Pantheon+ SN~Ia data points \cite{Scolnic2022} with both diagonal and full covariance errors, the CRM yields $\Delta\chi^2 = -12.2$ ($-11.2$) and $\Delta\mathrm{AIC} = -8.2$ ($-7.2$) relative to $\Lambda$CDM; the 5-fold cross-validation mean is slightly lower for CRM but, without fold-level variance estimates, should be treated as comparable rather than decisive. MCMC analysis gives $\Omega_m = 0.368 \pm 0.024$; four alternative saturation functions yield comparable results, confirming robustness. The phenomenological Pantheon+ fit produces a phantom-like effective diagnostic, $w_{\mathrm{eff}}(z)<-1$, but current DESI DR2 constraints favor $w_0>-1$, $w_a<0$ in the relevant $w_0w_a$CDM combinations \cite{DESI2025}; persistent phantomness is therefore treated as a tension to be resolved, not as a fundamental CRM prediction. The more robust late-time signature is a later acceleration onset ($z_{\mathrm{acc}} = 0.52$ vs.\ $0.84$), testable with Euclid, Roman, and DESI. The later acceleration implies an extended matter-dominated growth phase, naturally explaining the JWST ``Universe Breakers'' at $z > 7$ \cite{Labbe2023} and the El~Gordo anomaly \cite{Asencio2023}. Phantom pathologies are absent in the fitted ansatz because $\Omega_\Phi$ saturates ($\Omega_\Phi \to \Phi_0$, de~Sitter end state, no Big Rip). The game-theoretic axioms possess an exact thermodynamic equivalence connecting to the Jacobson tradition \cite{Jacobson1995}. Analysis code is publicly available.\footnote{\url{https://github.com/research-line/crm-cosmology}}
\end{abstract}
...
Recent results from the \textit{Dark Energy Spectroscopic Instrument} (DESI) strengthen doubts about a strictly constant dark energy: the DESI DR1 analysis of baryon acoustic oscillations combined with CMB and supernova data shows a 2.5--3.9$\sigma$ preference for a model with time-dependent equation-of-state parameter $w(z)$ over $\Lambda$CDM \cite{DESI2024}. The subsequent DESI DR2 result \cite{DESI2025} reinforces this finding with higher statistical power: in $w_0w_a$CDM it favors the quadrant $w_0>-1$, $w_a<0$, with supernova-dependent preferences over $\Lambda$CDM ranging from 2.8--4.2$\sigma$. For example, the DESI+CMB+Union3 combination gives $w_0=-0.667\pm0.088$, $w_a=-1.09^{+0.31}_{-0.27}$. This is in potential tension with the phantom-like effective diagnostic of the phenomenological CRM fit ($w_\mathrm{eff} < -1$); see the Limitations discussion (Sec.~\ref{sec:discussion}) for a detailed assessment.
...
\begin{enumerate}
\item There exists a metastable quantum vacuum state (hereafter: \textit{null space}) characterized by quantum fluctuations, with observable vacuum effects such as the Casimir force as the minimal physical analogy \cite{Casimir1948}. \textit{Ontological caveat:} The null space is a \emph{pre-geometric abstraction} -- a boundary condition for the variational problem -- not a concrete physical entity with independent spatiotemporal existence. Multiple UV-completion programs (Loop Quantum Gravity, Causal Set Theory, quantum error correction; see Paper~III \cite{Geiger2026c}, Appendix) are compatible with this abstraction.
...
This is the standard unilateral-deviation criterion introduced by Nash \cite{Nash1950}, used here only as an interpretive two-player potential-game template.
...
Within the phenomenological Pantheon+ tanh fit, the effective diagnostic in Table~\ref{tab:weff} is consistently below $w=-1$. This should not be read as a fundamental phantom-field prediction: it is a diagnostic of the fitted ansatz and must be confronted with the DESI DR2 preference for $w_0>-1$, $w_a<0$ and with the Lagrangian formulation of Paper~III. The relevant falsification test is therefore whether the curvature-relaxation sector can reproduce the late-time expansion history and the later acceleration transition without requiring a persistent physical phantom component.
...
The test is performed against the Pantheon+ data set \cite{Scolnic2022}, a large public catalog of spectroscopically confirmed Type~Ia supernovae. The release contains 1,701 light curves for 1,550 unique SNe~Ia; after applying the $z > 0.01$ mask used here to avoid peculiar-velocity dominance, 1,590 SN~Ia light-curve data points enter the fit, spanning $z = 0.0102$ to $z = 2.2614$. The observable is the bias-corrected apparent B-band magnitude \texttt{m\_b\_corr}. The analysis is performed both with diagonal errors and with the full statistical-systematic covariance matrix (STAT+SYS) of the Pantheon+ data set.
...
\textbf{Model selection:} In addition to $\chi^2$, the Akaike Information Criterion (AIC~$= \chi^2 + 2k$) and the Bayesian Information Criterion (BIC~$= \chi^2 + k \ln n$) are computed, where $k$ is the number of effective parameters and $n$ the number of data points. To check for overfitting, a 5-fold cross-validation is additionally performed. The complete analysis code is publicly available.\footnote{\url{https://github.com/research-line/crm-cosmology}}
...
\begin{table}[htbp]
\centering
\caption{Fitted parameters and goodness of fit: $\Lambda$CDM vs.\ CRM against Pantheon+ (1,590~SN~Ia data points after the $z>0.01$ mask). For CRM~(flat), $1\sigma$ MCMC uncertainties are given.}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
...
\textbf{Interpretation:} Two selection criteria favor the flat CRM over $\Lambda$CDM outright, $\chi^2$ ($-12.2$) and AIC ($-8.2$), while the raw cross-validation mean is slightly lower for CRM ($0.4499$ vs.\ $0.4519$). The BIC, which more strongly penalizes additional parameters, favors $\Lambda$CDM ($\Delta\mathrm{BIC} = +2.6$). According to the Kass--Raftery scale \cite{KassRaftery1995}, this lies in the ``positive evidence'' range against the more complex model. Cross-validation is the most direct overfitting check, but no fold-level standard deviation is reported; the 0.4\% difference should therefore be treated as comparable generalization, not as decisive evidence for CRM.
...
\textbf{Interpretation of $\Omega_m = 0.368$:} The CRM value exceeds the Planck CMB value ($\Omega_m^{\mathrm{Planck}} = 0.315 \pm 0.007$), which is typical for pure supernova fits. In the CRM context, this deviation has a possible physical interpretation: the model may be absorbing part of the density classified as ``dark energy'' in $\Lambda$CDM as a dynamic curvature effect. Since $\Omega_\Phi(a)$ vanishes at early times (unlike $\Omega_\Lambda = \mathrm{const.}$), $\Omega_m$ must be compensatorily higher to maintain the overall fit. This should not be read as independent support from weak-lensing surveys: lensing constraints primarily probe combinations such as $S_8$ and have their own tensions with Planck, so a joint SN+CMB+BAO+lensing fit is required before the higher SN-only $\Omega_m$ can be interpreted physically.
...
$f(R)$ gravity theories \cite{Starobinsky1980, Sotiriou2010} replace the Ricci scalar~$R$ in the Einstein--Hilbert action with a more general function. They offer a geometric explanation without dark energy but are mathematically complex and partly inconsistent with observations (gravitational lensing, CMB). The most widely studied late-time $f(R)$ model, Hu-Sawicki \cite{HuSawicki2007}, satisfies solar system constraints through the chameleon mechanism while producing viable cosmic acceleration; it serves as an important benchmark for the Lagrangian analysis in Paper~III \cite{Geiger2026c}. The purely quadratic $R^2$ term of the CRM does \textit{not} share this screening property, as discussed there.
...
\textbf{1.~Equation-of-state diagnostic:} The phenomenological Pantheon+ fit yields $|\Delta w| \approx 0.4$ relative to $\Lambda$CDM, but DESI DR2 moves the physical target away from persistent phantomness and toward a $w_0>-1$, $w_a<0$ trajectory. The ESA mission Euclid \cite{Euclid2024}, the Nancy Grace Roman Space Telescope, and DESI can test whether this is better described by curvature relaxation or by a generic $w_0w_a$CDM parametrization.
...
Roman & 2026--2027 & ${\approx}\,0.03$ & SN to $z \approx 2$ \\
...
\item \textbf{Empirical validation:} The CRM fits 1,590 Pantheon+ SN~Ia data points better than $\Lambda$CDM by $\chi^2$ and AIC ($\Delta\chi^2 = -12.2$, $\Delta\mathrm{AIC} = -8.2$) and shows comparable cross-validation performance (0.4499 vs.\ 0.4519, difference of 0.4\%).
...
\item \textbf{Reproducibility:} Analysis code and data are publicly available (\url{https://github.com/research-line/crm-cosmology}).
\end{enumerate}
...
\item \textbf{DESI DR2 tension and resolution via the Lagrangian theory:} The phenomenological Pantheon+ fit in this paper yields $w_{\mathrm{eff}} < -1$ (phantom-like diagnostic). However, DESI DR2 \cite{DESI2025} reports preferred $w_0w_a$CDM solutions in the quadrant $w_0>-1$, $w_a<0$; the published DESI+CMB+Union3 row gives $w_0=-0.667\pm0.088$, $w_a=-1.09^{+0.31}_{-0.27}$, while Pantheon+ and DESY5 combinations give different but still non-phantom $w_0$ values. We now treat this as a direct tension with the original phenomenological tanh reading and defer a quantitative resolution to the Lagrangian formulation in Paper~III~\cite{Geiger2026c}: the $f(R) = R + 2\gamma R^2$ sector is expected to approach $w=-1$ from above, rather than requiring a persistent physical phantom component. The value $w=-1.355$ is therefore a fitted effective diagnostic, not a prediction of the fundamental theory. Kovacs~\cite{Kovacs2026} is directionally relevant because its proposed covariant extension also lists $w\geq -1$ as a candidate consequence of the curvature-occupation kinetic structure, but that manuscript explicitly presents the cosmological extension as contingent and not yet a completed perturbation theory. We therefore regard $w\geq -1$ as the natural CRM target to test, not as a result already established by Paper~I.
\end{enumerate}
...
\item Solar system compatibility, by contrast, does \textit{not} follow from this chain. For a purely quadratic $f(R)$ the quantity $f_{RR}$ is constant, so the scalaron mass is curvature- and hence density-independent and no chameleon screening operates. An earlier version of this paper claimed such screening; that claim is withdrawn, and compatibility with local gravity tests is left open (Paper~III \cite{Geiger2026c}).
\end{itemize}
...
\textit{Scope of the series.} What this series claims is the background expansion history and the cosmological fits reported in Papers~I--III. The $f(R)$ perturbation sector and the local-gravity sector are \textit{not} claimed. Two points must be stated plainly. First, the analytic sector with $\gamma \sim H_0^{-2}$, from which the scalaron mass and the Compton wavelength are derived, does not satisfy the Cassini bound on the post-Newtonian parameter $\gamma_{\mathrm{PPN}}$: without a screening mechanism the fifth force is unsuppressed in the solar system (Paper~III \cite{Geiger2026c}). Second, none of the published fits of this series contains $\gamma$ as a parameter: the Planck fit of Paper~III varies $(\alpha_{M,0}, n)$ alongside the standard cosmological parameters, and the Pantheon+ fit of Paper~II varies $(\Omega_m, k, a_{\mathrm{trans}}, M)$. The cosmological fits are therefore carried by a separate effective-field-theory parametrization of the perturbation sector, not by a numerical evaluation of the analytic sector; the two are distinct parametrizations rather than two branches of a single calculation. Translating the constraint $\alpha_{M,0} \sim 10^{-3}$ into the $f(R)$ identification used here gives a coupling roughly four orders of magnitude smaller than the analytic sector assumes, and of opposite sign. Whether a single $f(R)$ model covers both regimes is an open question of this series and is not decided here.
...
\item The resulting \textit{Curvature Relaxation Model} (CRM) explains accelerated expansion without dark energy and passes the test against 1,590 Pantheon+ SN~Ia data points after the $z>0.01$ mask \cite{Scolnic2022}: $\Delta\chi^2 = -12.2$ (diagonal) / $-11.2$ (full covariance matrix), $\Delta\mathrm{AIC} = -8.2$ / $-7.2$, and comparable cross-validation performance (0.4499 vs.\ 0.4519).
\item Model selection is mixed: $\chi^2$ and AIC favor CRM, BIC favors $\Lambda$CDM, and 5-fold cross-validation is comparable but not decisive without fold-level variance estimates. Four alternative functional forms yield $\Delta\chi^2 \approx -9$ to $-12$, reducing but not eliminating the risk that the improvement reflects extra late-time flexibility.
\item MCMC-based parameter uncertainties ($\Omega_m = 0.368 \pm 0.024$) and the phantom stability analysis (no Big Rip, asymptotically de~Sitter) support phenomenological consistency, while publication-ready convergence diagnostics and a direct CPL/CMB/BAO comparison remain necessary.
\item The CRM makes testable predictions: a later acceleration transition ($z_{\mathrm{acc}} = 0.52$ vs.\ $0.84$ in $\Lambda$CDM) and a distinctive effective equation-of-state history that must be checked against DESI BAO, CMB, and supernova combinations rather than interpreted as persistent physical phantomness. Already now, the CRM prediction of an extended growth phase finds empirical support from JWST observations of unexpectedly massive galaxies at high redshifts \cite{Labbe2023, BoylanKolchin2023} and the statistically improbable existence of massive clusters such as El~Gordo \cite{Asencio2023}.
...
\href{https://doi.org/10.1016/j.dark.2026.102277}{DOI: 10.1016/j.dark.2026.102277}.
...
\href{https://doi.org/10.48550/arXiv.2503.15302}{DOI: 10.48550/arXiv.2503.15302}.

\bibitem{Casimir1948}
Casimir, H.\,B.\,G.\ (1948).
On the attraction between two perfectly conducting plates.
\textit{Proceedings of the Royal Netherlands Academy of Arts and Sciences}, 51, 793--795.
% No DOI available (published before DOI system).
...
\href{https://doi.org/10.48550/arXiv.2602.00422}{DOI: 10.48550/arXiv.2602.00422}.
