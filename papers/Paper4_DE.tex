...
\item Die Kopplungsfunktion $\mathcal{B}(\phi) = \mathrm{sech}^2(\phi/\phi_0)$ liefert automatische Abschirmung im Sonnensystem (Abschn.~6).
...
\begin{table*}[tbp]
\caption{\label{tab:galactic_status}Status der galaktischen Feldgleichungen im CRM-Vektorsektor. Drei Stufen epistemischer Zuversicht werden unterschieden.}
\begin{ruledtabular}
\begin{tabular}{ll}
\textbf{Kategorie} & \textbf{Ergebnis} \\
\hline
\textit{Etabliert:} Existenz & Gekoppeltes Skalar-Vektor-System erlaubt statische L\"osungen \\
\textit{Etabliert:} Skalierung & Deep-MOND $\sqrt{g_{\mathrm{bar}} a_0}$-Attraktor \\
