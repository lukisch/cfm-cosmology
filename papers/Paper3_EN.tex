...
\usepackage{float}
\usepackage{graphicx}
\graphicspath{{../figures/}{../figures/paper1/}{../figures/paper2/}}
...
\textbf{Units and notation.} Throughout the $f(R)$ sector we work in reduced Planck units ($8\pi G = 1$), in which the dimensionful $R^2$ coupling of the action~\eqref{eq:full_action} is $\epsilon \equiv 16\pi G\gamma = 2\gamma$. With this convention $f(R) = R + \epsilon R^2 = R + 2\gamma R^2$ and $f_{RR} = 2\epsilon = 4\gamma$, which fixes the coefficients of the trace equation~\eqref{eq:trace_equation} ($6\epsilon = 12\gamma$) and of the scalaron mass below. Explicit factors of $G$ are restored wherever dimensionful numerical values are quoted (Section~\ref{subsubsec:scalaron_mass}).

\textbf{Conformal equivalence.} The gravitational sector $f(R) = R + 2\gamma R^2$ is conformally equivalent to Einstein gravity plus a massive scalar field (the \textit{scalaron}) $\chi$:
\begin{equation}
...
\begin{equation}
m_{\mathrm{eff}}^2(a) = \frac{1}{12\gamma\,\mathcal{F}(a)}
\end{equation}
Since $\mathcal{F} \in [0,1]$ is monotonic and bounded, the mass remains real and positive at all times. At early times ($\mathcal{F} \to 0$), $m_{\mathrm{eff}} \to \infty$ and the scalaron is frozen out. At late times ($\mathcal{F} \to 1$), $m_{\mathrm{eff}} = m_s$, consistent with Eq.~\eqref{eq:scalaron_mass}.

\textbf{Newtonian limit.} In the weak-field limit, the scalaron produces a Yukawa correction:
\begin{equation}
V(r) = -\frac{GM}{r}\left(1 + \frac{1}{3}\,e^{-m_{\mathrm{eff}}\,r}\right)
\label{eq:yukawa}
\end{equation}
Solar system constraints require $m_{\mathrm{eff}}\,r_{\mathrm{AU}} \gg 1$. In the purely quadratic sector treated here, no chameleon mechanism is available to enforce this condition, and solar system compatibility is therefore not established; see the discussion of local gravity tests in Section~\ref{subsubsec:scalaron_mass}.
...
\begin{equation}
m_s^2 = \frac{1}{3}\left(\frac{f_R}{f_{RR}} - R\right) = \frac{1}{6\epsilon} = \frac{1}{12\gamma}\,, \qquad \gamma \geq \mathcal{O}(1)\,H_0^{-2}\,,
\label{eq:scalaron_mass}
\end{equation}
where the second equality is \textit{exact and curvature-independent} for $f = R + \epsilon R^2$: since $f_R/f_{RR} = 1/(2\epsilon) + R$, the curvature terms cancel identically, so the background value $R_0$ does not enter and the same mass applies in the cosmological background and in dense environments alike. This agrees with the value quoted in the ghost-freedom proposition above. Saturating the analytic-sector condition $\gamma \sim H_0^{-2}$ of Eq.~\eqref{eq:scalaron_mass} -- an assumption of that sector rather than a value inferred from the cosmological fits -- this yields:
\begin{equation}
m_s = \frac{1}{\sqrt{12\gamma}} \sim \frac{H_0}{\sqrt{12}} \approx 0.29\,H_0 \approx 4.2 \times 10^{-34}\,\mathrm{eV}\,,
\end{equation}
which is an upper bound, since $m_s \propto \gamma^{-1/2}$ decreases for $\gamma > \mathcal{O}(1)\,H_0^{-2}$. This corresponds to a cosmological Compton wavelength $\lambda_C = 2\pi/m_s \approx 9.6 \times 10^{4}\,\mathrm{Mpc} \approx 22\,c/H_0$, i.e.\ larger than the Hubble radius.

\textbf{Scale-dependent gravitational strength.} On sub-Compton scales ($k \gg a\,m_s$), the scalaron mediates a fifth force, enhancing gravity to $\mu \to 4/3$ (the $f(R)$ asymptotic value). On super-Compton scales ($k \ll a\,m_s$), the scalaron is too heavy to propagate, and GR is recovered ($\mu \to 1$). With $\lambda_C \approx 22\,c/H_0$, however, the transition lies beyond the Hubble radius, so that all observationally accessible scales are sub-Compton. Compatibility with large-scale structure can therefore not be argued from the Compton scale; assessing it requires the perturbation analysis of Section~\ref{subsec:perturbation_physics} and remains open here.

\textbf{Local gravity tests: no screening in the present sector.} Earlier versions of this paper claimed solar system screening by a chameleon mechanism \cite{Khoury2004}, derived from a density-dependent mass $m_{\mathrm{eff}}^2(\rho) = R(\rho)/(12\gamma)$. That claim is withdrawn. It rested on a spurious factor of $R$ in the scalaron mass; with the corrected Eq.~\eqref{eq:scalaron_mass} no such density dependence exists, because $f_{RR} = 2\epsilon$ is constant for a purely quadratic $f(R)$ and $m_s$ is therefore curvature- and hence density-independent.

This is structural rather than accidental. Chameleon screening in $f(R)$ dark energy models requires $f_{RR}$ to \textit{decrease} with curvature -- in the Hu--Sawicki class, $f_{RR} \propto R^{-(n+2)}$ \cite{HuSawicki2007} -- which a quadratic term cannot supply. Nor does the trace coupling substitute for it, and it points the other way: $\mathcal{F} = |T|/(|T| + \rho_{\mathrm{rad}}) \to 1$ in matter-dominated, dense environments, so that $m_{\mathrm{eff}} \to m_s$ inside the Sun rather than being enhanced. The role of $\mathcal{F}$ is to suppress the scalaron in the \textit{radiation} era (BBN protection), not in \textit{dense local} environments.

Consequently, compatibility with local gravity tests -- Lunar Laser Ranging ($\Delta G/G < 10^{-13}$) \cite{Williams2004} and the Cassini bound on the parametrized post-Newtonian parameter $\gamma_{\mathrm{PPN}} - 1 = (2.1 \pm 2.3) \times 10^{-5}$ \cite{Bertotti2003} -- is \textit{not} demonstrated for the Lagrangian~\eqref{eq:full_action} in its present form, and we make no such claim. A viable screening mechanism would require an additional, Hu--Sawicki-type term in $f(R)$, whose consequences for the cosmological sector are the subject of a separate investigation.
...
\end{equation}
where $\chi = f_R - 1 = 4\gamma\mathcal{F}(a) R$ is the scalaron field and $m_{\mathrm{eff}}^2(a) = 1/(12\gamma\mathcal{F}(a))$. The effective geometric energy density contributed by the scalaron is $\Omega_{R^2}(a) \propto \chi(a) \cdot R(a)$, and the effective scaling exponent follows as:
\begin{equation}
...
\begin{enumerate}
\item \textbf{Loop Quantum Gravity} \cite{Rovelli2004,Thiemann2007}: LQC's modified Friedmann equation $H^2 \propto \rho(1 - \rho/\rho_c)$ has the same $dX/dt \propto (1-X^2)$ structure as the saturation ODE. The bounded curvature invariants from holonomy corrections naturally produce saturation.

\item \textbf{Finsler Geometry} \cite{Bao2000,Girelli2007}: Direction-dependent metrics $F(x, \dot{x})$ produce scale-dependent gravitational effects (mimicking MOND) and non-standard cosmological scaling, potentially generating the $a^{-\beta}$ term from the osculating Riemannian curvature.

\item \textbf{Information-Theoretic Spacetime} \cite{Wheeler1990,Bousso2002}: The saturation ODE is the logistic growth equation for information processing, with $\Phi_0$ as the holographic capacity limit. Entanglement entropy saturation \cite{VanRaamsdonk2010} provides the microscopic mechanism.
...
\textbf{Superposition of all geometries:} Quantum-mechanically, the null space is the path-integral \cite{Feynman1948} over all possible spacetime configurations, weighted equally. It is the state of maximal uncertainty about geometry -- not ``empty space'' but ``no space at all.''
...
\label{eq:mu_ka}
\end{equation}
with $m_{\mathrm{eff}}^2(a) = 1/(12\gamma\,\mathcal{F}(a))$ the scalaron mass. At sub-horizon scales ($k \gg a\,m_{\mathrm{eff}}$), gravity is enhanced by a factor $4/3$; at super-horizon scales ($k \ll a\,m_{\mathrm{eff}}$), GR is recovered ($\mu \to 1$). The scalaron mass provides a natural \textit{scale-dependent screening}: small-scale perturbations ($k \gtrsim 0.1\,h/$Mpc) experience enhanced growth, while the large-scale CMB is nearly unaffected.
...
\begin{itemize}
\item Candidate~A (holographic noise) is \textbf{disfavored} by the Holometer null result \cite{Chou2017}, unless the noise is correlated (not random) as the CRM would predict.
\item Candidate~B (spin networks) is \textbf{mildly favored} by the cosmic birefringence hint.
\item Candidate~C (entanglement) is \textbf{constrained} but not excluded; the simple models fail \cite{Donadi2021}, but more sophisticated entanglement-saturation models remain viable.
...
The action~\eqref{eq:full_action} with the $R^2$ term is a special case of $f(R) = R + \gamma R^2$ gravity (Starobinsky model) \cite{Starobinsky1980}. A large body of work has explored $f(R)$ models as alternatives to dark energy, most notably the Hu-Sawicki model \cite{HuSawicki2007}, which was designed to satisfy solar system constraints through the chameleon mechanism while producing viable late-time acceleration. The CRM does \textit{not} share this chameleon property -- a purely quadratic $R^2$ term has constant $f_{RR}$ and therefore cannot screen (Section~\ref{subsubsec:scalaron_mass}) -- and it also differs in motivation: while Hu-Sawicki engineers $f(R)$ to mimic $\Lambda$CDM at the background level, the CRM derives $R + \gamma R^2$ from the curvature relaxation mechanism (Paper~I) and adds the scalar field with the P\"oschl-Teller potential, breaking the degeneracy between $f(R)$ models. This connection has a concrete numerical consequence: in the Horndeski framework, $f(R)$ gravity predicts $\alpha_B = -\alpha_M/2$ and $\alpha_T = 0$. Using hi\_class \cite{Zumalacarregui2017} with this exact relation ($\alpha_M = 0.0007$), the CRM achieves $\ell_1 = 220$ and $\mathcal{P}_3/\mathcal{P}_1 = 0.4295$ (both exact Planck, directly verified) through the early ISW effect, providing direct numerical evidence that the $R^2$ structure of the CRM Lagrangian produces the correct perturbation physics.
...
\end{align}
The posterior satisfies $P(\alpha_{M,0} > 0) = 99.99\%$. This figure must not be read as evidence on the \textit{sign} of $\alpha_{M,0}$: the flat prior is restricted to $\alpha_{M,0} \in [0,\,0.003]$ (Table~\ref{tab:priors}), so negative values were excluded by construction and the probability reproduces the prior rather than testing it. How strongly the data prefer a non-zero modification is measured by $\Delta\chi^2$ and by the $1.8\sigma$ figure discussed below, not by this number. The standard cosmological parameters ($\omega_{\mathrm{cdm}}$, $A_s$, $n_s$) are essentially uncorrelated with the modified gravity parameters ($|\rho| < 0.09$), confirming that the cfm\_fR extension is a clean, perturbative addition to $\Lambda$CDM. The strong anti-correlation between $\alpha_{M,0}$ and $n$ ($\rho = -0.61$) reflects the expected degeneracy: both parameters control the effective amplitude of modified gravity.
...
$P(\alpha_{M,0} > 0)$ & 99.99\% (prior-limited: $\alpha_{M,0} \geq 0$ imposed; not a sign test) & --- & --- \\
...
It is also instructive to compare the cfm\_fR model with the widely studied Hu-Sawicki $f(R)$ model \cite{HuSawicki2007}, which was designed to satisfy solar system constraints through a chameleon mechanism while producing late-time acceleration. The Hu-Sawicki model is parametrized by $|f_{R0}|$, the present-day value of the scalaron field. Planck constraints give $\log_{10}|f_{R0}| < -4.79$ (95\% CL) \cite{PlanckMG2016}. The CRM Lagrangian $R + \gamma R^2$ lacks the chameleon screening property of that model (Section~\ref{subsubsec:scalaron_mass}) and differs further in that the $R^2$ modification is \textit{universal} (not scale-dependent by construction), and the running is controlled by the scalaron dynamics rather than by an engineered functional form.
...
\item \textbf{Ghost freedom and Newtonian limit:} The action~\eqref{eq:full_action} is proven ghost-free (Section~\ref{subsec:ghost_analysis}): $f_{RR} > 0$ excludes the Ostrogradsky ghost and $m_s^2 = 1/(12\gamma) > 0$ excludes tachyonic instabilities. Solar system screening, by contrast, is \textit{not} established: for a purely quadratic $f(R)$ the quantity $f_{RR}$ is constant, so the scalaron mass is density-independent and no chameleon mechanism operates. Compatibility with local gravity tests therefore remains an open question (Section~\ref{subsubsec:scalaron_mass}).

\item \textbf{Lagrangian derivation of $\beta_{\mathrm{eff}}(a)$:} The running coupling emerges from the scalaron equation of motion~\eqref{eq:scalaron_eom} with time-dependent mass $m_{\mathrm{eff}}^2(a) = 1/(12\gamma\mathcal{F}(a))$ (Section~\ref{subsec:beta_derivation}). The phenomenological sigmoidal parametrization approximates the numerical solution. \textit{Note:} The running $\mu(a)$ of Paper~II remains a phenomenological transition function; no Lagrangian derivation of $\mu(a)$ is claimed.
...
available at \url{https://github.com/research-line/crm-cosmology}
...
\href{https://doi.org/10.1017/CBO9780511755804}{DOI: 10.1017/CBO9780511755804}.
...
% No DOI available (conference proceedings).
...
% No DOI available (book chapter, pre-DOI era).
...
\href{https://doi.org/10.48550/arXiv.2601.14559}{DOI: 10.48550/arXiv.2601.14559}.
...
\href{https://doi.org/10.48550/arXiv.2602.12238}{DOI: 10.48550/arXiv.2602.12238}.
...
\href{https://doi.org/10.1093/mnras/stad3107}{DOI: 10.1093/mnras/stad3107}.
...
Cambridge University Press. ISBN: 978-0-521-65476-0.

\bibitem{ZhongPT2022}
Zhong, Y.\ (2022).
Singular P\"oschl-Teller Approximation for Black Hole Quasinormal Modes.
\textit{European Physical Journal C}, 82, 511.
% DOI not verified -- paper could not be located in EPJC with these details.
...
\href{https://doi.org/10.48550/arXiv.2601.00704}{DOI: 10.48550/arXiv.2601.00704}.
