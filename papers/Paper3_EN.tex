...
\end{equation}
where $R_0 \approx 3H_0^2(4\Omega_\Lambda + \Omega_m) \approx 9.2\,H_0^2$ is the background curvature today. Saturating the constraint $\gamma \sim H_0^{-2}$ (implied by $\alpha_{M,0} \sim 10^{-3}$), this yields:
\begin{equation}
m_s \sim \sqrt{\frac{9.2}{12}}\,H_0 \sim 0.88\,H_0 \sim 10^{-33}\,\mathrm{eV}\,,
\end{equation}
which is an upper bound, since $m_s \propto \gamma^{-1/2}$ decreases for $\gamma > \mathcal{O}(1)\,H_0^{-2}$. This corresponds to a cosmological Compton wavelength $\lambda_C = 2\pi/m_s \gtrsim \mathcal{O}(100)\,\mathrm{Mpc}$.
