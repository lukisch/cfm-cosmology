...
\textit{Physical content.} This axiom imposes finite capacity for the selected scalar response channel. It is not a claim that every microscopic observable or every UV amplitude is finite in the underlying theory. It is deliberately weak: it is compatible with LQG (finite area quanta \cite{Rovelli2004}), holography (Bekenstein bound \cite{Bekenstein1981}), quantum error-correcting spacetime (finite code capacity \cite{Almheiri2015}), causal set theory (finite density \cite{Sorkin2003}), and asymptotic safety (finite effective action at the UV fixed point \cite{Reuter1998}) when those programs are restricted to the chosen cosmological response channel.
...
\]
Thus bare smoothness of the positive saturation face places the response in an Abel-coordinate collar framework; it does not classify all smooth collars or select a unique projective coordinate. Boundary smoothness is used as an admissibility condition for the response channel. It is not derived from a UV model in this paper. The $\tanh$ normal form becomes a theorem only after the projective double-boundary quotient D' is fixed explicitly.
\end{remark}
...
\label{eq:abel}
\end{equation}
This is Abel's functional equation. \emph{(Proof step corrected in this revision; an earlier version wrote $\phi'(0)=c_1$ from Axiom~B, which was a category error: $c_1$ is the derivative of the response profile $\sigma$ at the control-parameter origin, not of the Abel coordinate $\phi$ at the response origin.)} The clean construction proceeds via the invariant vector field: let $a(x) = \partial_2 C(x,0)$; under $D^\pm$, translations are orientation-preserving diffeomorphisms, hence $a(x) > 0$ on $(-1,1)$, and one may define $\phi(x) = \int_0^x dt/a(t)$. The invariance of $a$ under the group law then gives $\phi(C(x,y)) = \phi(x) + \phi(y)$, and $\phi$ inherits the $C^\infty$ regularity of $C$ on $(-1,1)$ directly from this integral representation. The function $\phi$ is determined up to a positive multiplicative constant, and antisymmetry (Axiom~B') lets it be chosen odd; Axiom~B supplies $\sigma'(0) = c_1 > 0$, which enters only through the product $(\phi\circ\sigma)'(0) = \phi'(0)\,c_1$ fixing the overall positive scale $\alpha$. At this stage Axioms~A--D give an Abel/collar theorem, not yet $\phi=\mathrm{arctanh}$.
...
\begin{corollary}[Monotonicity from the reversible response channel]
\label{cor:monotonicity}
Under the reversible-channel hypothesis $D^\pm$, Axiom~C (monotone cosmic control) follows from Axioms~A, B, B', and~D. That is, any response function satisfying $A + B + B' + D^\pm$ is automatically strictly monotonically increasing on the one-dimensional channel.
\end{corollary}
...
\textit{(3) Violates C (and A on the positive branch), satisfies B/B' locally but not $D^\pm$:} $\sigma(g) = \tanh(g) \cdot \cos(\epsilon g)$ for small $\epsilon > 0$. \emph{Correction:} an earlier version claimed this example ``satisfies A''; in fact, for $g \gg 1$ the factor $\cos(\epsilon g)$ changes sign, so on the positive branch the example also violates the nonnegativity of Axiom~A and possesses no saturation limit in the sense of Axiom~C. It therefore separates $\{$C$\}$ (and monotone saturation generally) from local $\{$B,B$'\}$, but it is \emph{not} a witness that C is independent of A. A cleaner candidate witness for C-independence -- a smooth, odd, bounded profile of the form $\sigma(g) = \tanh(g) + \varepsilon\,\psi(g)$ with $\psi$ a compactly supported odd bump chosen so that $0 \leq \sigma < 1$ on $g \geq 0$ but $\sigma' < 0$ on some interval -- should be verified in a separation ledger before being asserted. By Corollary~\ref{cor:monotonicity}, C is in any case \textit{not} independent of $\{A, B, B', D^\pm\}$ jointly -- it follows as a derived consequence under the interior group law. C is retained for physical clarity and to make the single-channel arrow explicit.
...
Asymptotic safety is therefore one of the closest matches to Axiom~D, because its central language is renormalization-group evolution.\footnote{The self-similar, scale-recursive character of Axiom~D is not
...
motivating parallel, not as an independent verification of Axioms~A--D.} The asymptotic safety scenario for cosmology \cite{Bonanno2002} produces effective Friedmann equations with saturation behavior at high curvature, consistent with the $\tanh$ form.
