...
\textbf{Lagrangian origin from $f(R) = R + 2\gamma R^2$:} Paper~III \cite{Geiger2026c} demonstrates that the trace-coupling suppression $\mathcal{S}(a)$ is \textit{not} an ad~hoc postulate but follows rigorously from the $R^2$ sector of the CRM Lagrangian. This places the construction in the same $f(R)$-gravity landscape as Hu--Sawicki screening models \cite{HuSawicki2007}, while retaining the CRM-specific saturation motivation; the kinship is one of setting only, since the purely quadratic CRM term does not reproduce the chameleon screening of that class \cite{Geiger2026c}. The trace of the field equations for $f(R) = R + 2\gamma R^2$ gives:
\begin{equation}
R + 12\gamma\,\Box R = -8\pi G\,T
\label{eq:trace_fR}
\end{equation}
where $T = g^{\mu\nu}T_{\mu\nu}$ is the matter energy-momentum trace. During the radiation era, $T_{\mathrm{rad}} = 0$ (conformal symmetry), which implies $R + 12\gamma\,\Box R = 0$. On FLRW, the decaying solution gives $R \to 0$ in the radiation era, so the $R^2$ correction -- which sources the scalaron and hence the geometric DM term -- vanishes automatically. The suppression factor $\mathcal{S}(a)$ in Eq.~\eqref{eq:suppression} is thus the phenomenological parametrization of a rigorous $f(R)$ result: the geometric DM ``activates'' precisely when $T \neq 0$ (i.e., when non-relativistic matter dominates). The only genuine postulate is the choice $f(R) = R + 2\gamma R^2$ -- the trace coupling is a \textit{consequence}, not an input. Paper~III also derives the scalaron mass directly from the $f(R)$ function, $m_s^2 = 1/(12\gamma)$, which is exact and curvature-independent for a purely quadratic $f(R)$; saturating $\gamma \sim H_0^{-2}$ gives $m_s \approx 0.29\,H_0 \approx 4.2 \times 10^{-34}\,\mathrm{eV}$ as an upper bound, and a cosmological Compton wavelength $\lambda_C \approx 9.6 \times 10^{4}\,\mathrm{Mpc}$, i.e.\ larger than the Hubble radius \cite{Geiger2026c}. Earlier versions of this paper instead quoted $m_s \sim \mathcal{O}(10)\,H_0$, $\lambda_C \gtrsim 100\,\mathrm{Mpc}$ and a solar system screening length $\lambda_C^{\mathrm{solar}} \sim 20\,\mathrm{m}$ from a chameleon mechanism, and concluded compatibility with local gravity tests. Those values and that conclusion are withdrawn: since $f_{RR}$ is constant for a quadratic term, the scalaron mass is density-independent and no chameleon screening operates, so compatibility with local gravity tests is not established and remains open \cite{Geiger2026c}.
...
To keep the empirical argument auditable, Table~\ref{tab:evidence_gate_ledger} separates the current background-level support from the remaining theory and perturbation gates. This prevents the SN-only result from being read as a completed full-cosmology validation.

\begin{table*}[t]
\centering
\caption{Evidence-gate ledger for the main data layers in Paper II.}
\label{tab:evidence_gate_ledger}
\footnotesize
\begin{tabular}{@{}llll@{}}
\toprule
\parbox[t]{0.17\textwidth}{\raggedright \textbf{Layer}} & \parbox[t]{0.21\textwidth}{\raggedright \textbf{Data object}} & \parbox[t]{0.26\textwidth}{\raggedright \textbf{Current presentation}} & \parbox[t]{0.24\textwidth}{\raggedright \textbf{Open gate}} \\
\midrule
\parbox[t]{0.17\textwidth}{\raggedright SN background fit} & \parbox[t]{0.21\textwidth}{\raggedright 1,590 Pantheon+ SNe Ia with $z>0.01$} & \parbox[t]{0.26\textwidth}{\raggedright Model comparison and MCMC posterior support a curvature-like term with $\Delta\chi^2=-26.3$ vs.\ $\Lambda$CDM.} & \parbox[t]{0.24\textwidth}{\raggedright Repeat with alternative SN covariance choices and independent low-redshift systematics cuts.} \\
\parbox[t]{0.17\textwidth}{\raggedright Out-of-sample check} & \parbox[t]{0.21\textwidth}{\raggedright 5-fold Pantheon+ cross-validation} & \parbox[t]{0.26\textwidth}{\raggedright Held-out predictive $\chi^2/n$ is slightly lower for Extended CRM+MOND than for $\Lambda$CDM.} & \parbox[t]{0.24\textwidth}{\raggedright Stress-test folds, seeds, and nuisance treatment before treating the gain as robust generalization.} \\
\parbox[t]{0.17\textwidth}{\raggedright Compressed joint background} & \parbox[t]{0.21\textwidth}{\raggedright Pantheon+ plus Planck distance priors and 9 BAO points} & \parbox[t]{0.26\textwidth}{\raggedright The running-coupling variant reaches zero EDE and a competitive joint background fit.} & \parbox[t]{0.24\textwidth}{\raggedright Replace compressed priors with a full Boltzmann and CMB likelihood analysis.} \\
\parbox[t]{0.17\textwidth}{\raggedright Running coupling and MOND bridge} & \parbox[t]{0.21\textwidth}{\raggedright Optimized $\beta_{\mathrm{eff}}(a)$ and $\mu(a)$ profiles} & \parbox[t]{0.26\textwidth}{\raggedright Later tables show that scale dependence can remove the CMB sound-horizon failure of constant $\beta$.} & \parbox[t]{0.24\textwidth}{\raggedright Derive the profiles from a covariant action and test stability across redshift.} \\
\parbox[t]{0.17\textwidth}{\raggedright Perturbation and lensing sector} & \parbox[t]{0.21\textwidth}{\raggedright Boltzmann-code path, lensing, and Bullet-Cluster logic} & \parbox[t]{0.26\textwidth}{\raggedright Presented as a validation program rather than as a completed dataset-level test.} & \parbox[t]{0.24\textwidth}{\raggedright Provide CRM perturbation equations and compare against CMB spectra, growth, and lensing surveys.} \\
\bottomrule
\end{tabular}
\end{table*}
...
\item \textbf{BAO:} 9 distance measurements from 6dFGS ($z = 0.15$), BOSS DR12 \cite{Alam2017} ($z = 0.38, 0.51, 0.61$), and Lyman-$\alpha$ ($z = 2.334$).
\end{enumerate}
...
\begin{enumerate}
\item Native CRM gravity model in hi\_class with time-dependent $\alpha_M(a)$ from the curvature relaxation physics, or an EFTCAMB implementation \cite{Hu2014}. The current analysis using \texttt{constant\_alphas} achieves $\ell_1 = 220$ and $\mathcal{P}_3/\mathcal{P}_1 = 0.4295$ (both exact Planck), but the constant parametrization encounters numerical instabilities at high $\omega_c$. A time-dependent implementation (\texttt{eft\_alphas\_power\_law} or custom model) would resolve this and close the remaining $\theta_s$ offset (1.035 vs.\ 1.041, 0.63\%).
