...
This paper extends the Curvature Relaxation Model (CRM) from the cosmological (Papers~I--II) and scalar-field (Paper~III) scales to galactic dynamics. The central result is that the CRM's curvature-saturation mechanism, when coupled to a massive vector field, reproduces the empirical MOND phenomenology---including the characteristic acceleration scale $a_0 \approx cH_0/(2\pi)$---without postulating dark matter or modifying Newtonian gravity ad hoc. The key ingredients are: (i)~a scalar--vector Lagrangian derived from the thermodynamic equilibrium of Paper~I; (ii)~a BVP-extracted interpolation function $\mu(x)$ that outperforms the McGaugh form; (iii)~a treatment of CMB and gravitational-wave constraints. Solar-system compatibility is \textit{not} among the results: it was previously argued from chameleon screening of the scalaron, and that argument is withdrawn (Sec.~\ref{sec:screening}). \textit{Reader Guide:} Sections~1--4 develop the formalism; Sections~5--6 address screening and its open status; Sections~7--8 present galactic predictions; Section~9 discusses open problems.
...
\item \textbf{Paper~III:} The effective Lagrangian $\mathcal{L}_{\mathrm{T1}}$ for the scalar sector, with $f(R) = R + \gamma R^2$, P\"oschl-Teller saturation and trace coupling. Perturbation analysis yields $\mu(k,a) \to 4/3$ at sub-Compton scales and $\alpha_T = 0$ exactly \cite{Geiger2026c}. The solar-system consistency once quoted from that paper ($m_{\mathrm{eff}}^{\mathrm{solar}}/m_s \sim 4 \times 10^{14}$) has since been withdrawn there: for a purely quadratic $f(R)$ the quantity $f_{RR}$ is constant, so the scalaron mass is density-independent and the ratio is in fact unity.
\end{itemize}
...
\item The screening architecture is twofold (scalar mass + vector decoupling), but its scalar half is not delivered by the quadratic $f(R)$ sector; robustness against Solar System tests is therefore assumed, not demonstrated (Sec.~\ref{sec:screening}).
\end{enumerate}
...
The scalaron (scalar sector) modifies gravity on cosmological scales through the $f(R) = R + \gamma R^2$ extension. On galactic and smaller scales, this modification is assumed to be suppressed by Chameleon screening. Earlier versions of this paper quantified the suppression as a rise of the effective scalaron mass by $\sim 4 \times 10^{14}$ in the Solar System, confining the fifth force to Compton wavelengths of $\sim 20$\,m. Those figures came from Paper~III and have been withdrawn there \cite{Geiger2026c}; in the purely quadratic sector the scalaron mass does not depend on density at all. The suppression is retained below only as a working assumption, and everything that rests on it is conditional (Sec.~\ref{sec:screening}).
...
From the thermodynamic/emergent-gravity perspective of Paper~I, in the Jacobson--Verlinde tradition \cite{Jacobson1995,Verlinde2017}:
\begin{enumerate}
...
This suggests that MOND effects should \textit{weaken over cosmic time} as the universe approaches thermodynamic equilibrium. We classify this as a \emph{testable but optional consequence} of the framework: the weakening follows from $\mathcal{B}(\phi) \to 0$ as $\phi \to 0$ (Sec.~\ref{sec:bvp_results}), but the timescale and magnitude are model-dependent and potentially degenerate with baryonic evolution effects (star formation efficiency, gas accretion history). High-redshift galaxy surveys (JWST, ELT) can constrain the effect, but a non-detection would not falsify the CRM -- it would constrain the scalar field evolution rate. This MOND-scale weakening should be regarded as a prediction contingent on the specific vector-field configuration rather than a robust model-independent result.

\textit{Note on the status of the Principal-Agent framework:} The principal-agent construction is a heuristic interpretive framework, not a formal physical derivation. The prediction of MOND weakening over cosmic time does not follow mathematically from the principal-agent picture alone; it follows from the dynamics of the CRM scalar field $\phi$ evolving toward $\phi = 0$ (null state), which reduces the coupling strength $\mathcal{B}(\phi) \to 0$ at late times (Sec.~\ref{sec:bvp_results}). The principal-agent language provides intuition for \textit{why} this should occur but is not required for the formal argument. Readers should not interpret the equilibrium of the principal-agent metaphor as implying that MOND is formally \textit{derived} from this picture; it is suggested by it.
...
\label{eq:S_T1}
\end{align}
where $V_{\mathrm{PT}}(\phi) = V_0/\cosh^2(\phi/\phi_0)$ is the P\"oschl-Teller potential providing saturation dynamics, $F(T/\rho)$ is the trace-coupling function (vanishing during radiation domination to protect BBN), and $\gamma \sim \mathcal{O}(H_0^{-2})$ is the value assumed for the analytic sector of Paper~III \cite{Geiger2026c}; it is not a parameter of the cosmological fits.
...
We emphasize that the vector sector presented here constitutes a \emph{minimal consistent realization} of the daughter-2 dynamics, not a unique derivation. Alternative vector-field configurations satisfying the same boundary conditions may exist; the present construction is selected for its economy and compatibility with existing observational constraints.
...
\textit{Independent corroboration.} An independent thermodynamic derivation by Brodie~\cite{Brodie2026} obtains $a_0 = cH_0/6 \approx 1.09 \times 10^{-10}$\,m/s$^2$ from entanglement sharing between the local Rindler horizon and the cosmological Hubble horizon. The geometric factor $1/6$ arises from the angular overlap integral $\int \cos^2\theta\,d\Omega$ over the hemisphere. Both analytical predictions -- $1/(2\pi) \approx 0.159$ (CRM) and $1/6 \approx 0.167$ (Brodie) -- lie within the empirical uncertainty of $a_0^{\mathrm{obs}}$ and differ by only $5\%$ from each other. That two conceptually distinct thermodynamic frameworks (scalar-field Fourier duality vs.\ horizon entanglement partition) independently reproduce $a_0$ without free parameters strongly suggests that $a_0 \sim cH_0$ reflects genuine physics rather than numerical coincidence. A discriminating test using the full SPARC database -- comparing $\chi^2$ for $a_0 = cH_0/(2\pi)$ vs.\ $a_0 = cH_0/6$ vs.\ free $a_0$ -- is left for future work. For a comprehensive phenomenological review of the $a_0 \sim cH_0$ coincidence, see Famaey \& McGaugh~\cite{FamaeyMcGaugh2012}.
...
In the Newtonian regime ($g \gg a_0$), the Chameleon mechanism is assumed to suppress both the scalar and vector contributions (see Sec.~\ref{sec:scalar_screening_status} for the status of that assumption):
\begin{itemize}
...
A density-dependent scalaron mass would provide the switching criterion:
\begin{itemize}
...
\textit{Status.} This is the switch on which the Newtonian--MOND transition of the present paper turns, and it is an assumption rather than a result. The Chameleon mechanism that was previously invoked to supply it does not exist in the quadratic $f(R)$ sector of Paper~III (Sec.~\ref{sec:scalar_screening_status}). The three-regime picture above is therefore retained as the working hypothesis under which the numerical results of Secs.~\ref{sec:nonlinear_origin} and~\ref{sec:bvp_results} were obtained; identifying a mechanism that actually generates it is an open problem of the model, not a settled part of it.
...
\subsection{Scalar sector screening: status of the claim}
\label{sec:scalar_screening_status}

Earlier versions of this paper took the Chameleon mechanism for the scalaron to have been established quantitatively in Paper~III:
\begin{equation}
\frac{m_{\mathrm{eff}}^{\mathrm{solar}}}{m_s} \sim 4 \times 10^{14}
\label{eq:meff_solar}
\end{equation}
yielding a Compton wavelength $\lambda_C \sim 20$\,m in the Solar System, far below any measurable gravitational scale, and hence consistency with Cassini ($\gamma_{\mathrm{PPN}} - 1 = (2.1 \pm 2.3) \times 10^{-5}$) \cite{Bertotti2003} and Lunar Laser Ranging ($\Delta G/G < 10^{-13}$).

\textit{That claim is withdrawn.} Paper~III has since retracted the result on which it rested \cite{Geiger2026c}: the density dependence of the scalaron mass came from a spurious curvature factor. For $f(R) = R + \gamma R^2$ the quantity $f_{RR}$ is constant, so $m_s$ is curvature- and therefore density-independent, and the ratio in Eq.~\eqref{eq:meff_solar} is in fact unity rather than $4 \times 10^{14}$. This is structural, not a numerical slip: chameleon screening in $f(R)$ dark energy models requires $f_{RR}$ to \textit{decrease} with curvature -- $f_{RR} \propto R^{-(n+2)}$ in the Hu--Sawicki class \cite{HuSawicki2007} -- which a purely quadratic term cannot supply. Nor does the trace coupling substitute for it: $\mathcal{F} \to 1$ in dense, matter-dominated environments, so the scalaron is not made heavier inside the Sun.

Consequently, compatibility of the scalar sector with local gravity tests is \textit{not} established, and we make no such claim. Equation~\eqref{eq:meff_solar} is retained in what follows only to identify the assumption under which the subsequent estimates were computed; every result that depends on it is conditional on an extension of $f(R)$ that supplies genuine screening -- a Hu--Sawicki-type term being the obvious candidate -- whose consequences for the cosmological sector are the subject of a separate investigation.

\subsection{Parasitic vector screening (conditional)}
\label{sec:parasitic}

The estimate of this subsection is inherited from the scalar sector and therefore inherits its conditional status (Sec.~\ref{sec:scalar_screening_status}); it is presented as it stood, followed by an assessment of what survives.
...
Within its own premise this estimate is conservative: the linearized Yukawa profile overestimates $\partial_r\phi$ relative to the full nonlinear Chameleon solution, which features a ``thin shell'' effect that further suppresses the exterior gradient \cite{HuSawicki2007}.

\textit{What survives.} The inheritance argument is sound, but it can only inherit what the scalar sector provides, and by Sec.~\ref{sec:scalar_screening_status} the scalar sector provides nothing. With a density-independent scalaron mass, $m_{\mathrm{eff}}\,r$ at $1$\,AU is not $7.5 \times 10^{9}$ but negligibly small, so the Yukawa factor in Eq.~\eqref{eq:parasitic} is of order unity and the density enhancement $|T|/\rho_{\mathrm{crit}} \sim 10^{30}$ stands unopposed. Solar-system safety of the vector sector is therefore \textit{not} established either, and the numerical values quoted in this subsection are no longer load-bearing. What remains valid is the structural statement -- the vector field couples to $\partial_\mu\phi$ rather than to matter, so wherever the scalaron gradient is suppressed the vector source is suppressed with it. Turning that structure into a solar-system bound requires a screening mechanism that the present Lagrangian does not contain.

\textit{Quantitative bridge to galactic scales:} A reviewer concern is that the near-perfect Solar System suppression ($\sim 10^{-3 \times 10^9}$) raises the question of why the vector sector can still contribute significantly in galaxies. The mechanism is density-dependent: in the Solar System interior ($\rho_\odot \sim 1400$\,kg/m$^3$), the Chameleon effective mass is $m_\mathrm{eff}/m_s \sim 4 \times 10^{14}$ (Eq.~\eqref{eq:meff_solar}), giving a Compton wavelength $\lambda_C \sim 20$\,m. In the galactic outskirts ($\rho_\mathrm{gal} \sim 10^{-26}$\,kg/m$^3 \approx \rho_\mathrm{crit}$), the Chameleon screening is lifted: $m_\mathrm{eff}/m_s \to 1$, giving $\lambda_C \sim \mathrm{kpc}$, comparable to galactic dynamical scales. The transition between these two regimes is sharp (thin-shell effect), ensuring both Solar System consistency and galactic-scale MOND effects simultaneously. What was described in earlier versions as a missing numerical bridge -- the transition density $\rho_\mathrm{screen}$ and the profile of $\partial_r\phi$ across the full density range -- is in fact a missing mechanism: by Sec.~\ref{sec:scalar_screening_status} the quadratic sector produces no density dependence at all, so there are not two regimes to bridge. Supplying one is an open theoretical task, not merely an open computation.
...
\begin{table*}[tbp]
\caption{\label{tab:termination}Termination hierarchy. Each sector is active only where its contribution is needed for entropy production or structural stability. The ``Screened'' and ``Decoupled'' entries in the Solar System row are conditional: they presuppose the density-dependent scalaron mass discussed in Sec.~\ref{sec:scalar_screening_status}, which the quadratic $f(R)$ sector does not supply.}
\begin{ruledtabular}
\begin{tabular}{llll}
...
The linearized result $g_A \propto g_N$ corresponds to a constant enhancement factor and is insufficient to produce MOND phenomenology. The $\sqrt{\phantom{x}}$-scaling requires nonlinear feedback, whose origin we identify in Sec.~\ref{sec:nonlinear_origin}.

\textit{Caveat on the linearized formalism (critical limitation):} The derivation above establishes only that the CRM vector sector \emph{can} contribute an additional acceleration. It does \emph{not} establish MOND phenomenology: a constant rescaling $g_A \propto g_N$ yields $g_\mathrm{obs} = (1+\alpha)g_N$, which preserves the $1/r^2$ fall-off and does not produce flat rotation curves. The MOND $\sqrt{g_N\,a_0}$ attractor claimed in subsequent sections depends entirely on the nonlinear feedback mechanism (Sec.~\ref{sec:nonlinear_origin}), which has not been derived analytically from the CRM action. Until that derivation is completed, the claim that CRM reproduces MOND phenomenology rests on a plausibility argument (the feedback loop structure), not on a rigorous calculation. This is a substantive open gap in the current paper.

If the deep-MOND attractor is realized (as expected from the structure of the feedback loop), the consequences are:
\begin{itemize}
...
\end{equation}
where $m_{\mathrm{eff}}(\rho)$ is \textit{assumed} to increase rapidly in high-density environments and to approach a small cosmological background value in the low-density outskirts. The screened-branch value once taken from Paper~III ($m_{\mathrm{eff}}^{\mathrm{solar}}/m_s \sim 4 \times 10^{14}$) has been withdrawn there \cite{Geiger2026c}; see Sec.~\ref{sec:scalar_screening_status}. Since the vector contribution satisfies $\Xi = \Xi[\varphi', \dot{\bar{\phi}}]$ (Eq.~\ref{eq:Xi_def}), the coupled system forms a closed nonlinear feedback loop:
\begin{equation}
...
In the screened regime ($m_{\mathrm{eff}}\,r \gg 1$), the scalar gradient is exponentially suppressed, the vector source vanishes by parasitic screening (Sec.~\ref{sec:parasitic}), and pure GR is recovered: $g \simeq g_N$. In the unscreened regime ($m_{\mathrm{eff}}\,r \ll 1$), the scalar responds efficiently to the baryonic source, the vector sector becomes active, and the feedback loop~\eqref{eq:feedback_loop} allows a MOND-like attractor. The transition between these regimes is controlled by the Chameleon mass profile. \textit{Caveat.} The whole of this mechanism presupposes a density-dependent $m_{\mathrm{eff}}(\rho)$, which Sec.~\ref{sec:scalar_screening_status} shows the present Lagrangian does not provide; the nonlinear origin identified here is therefore conditional on an extension of $f(R)$ that does. It is worth noting separately that the value of $a_0$ is \textit{not} obtained from this profile: the consistency relation $a_0 = cH_0/(2\pi)$ of Sec.~\ref{sec:a0_derivation} follows from the quasi-static scalar dynamics and is untouched by the present caveat.
...
\begin{table*}[tbp]
\caption{\label{tab:galactic_status}Status of the galactic field equations in the CRM vector sector. Four levels of epistemic confidence are distinguished. ``Conditional'' marks a result computed under an assumed density-dependent effective mass $m_{\mathrm{eff}}(\rho)$; that assumption is not supplied by the quadratic $f(R)$ sector, and its status is discussed in Sec.~\ref{sec:scalar_screening_status}.}
\begin{ruledtabular}
\begin{tabular}{ll}
\textbf{Category} & \textbf{Result} \\
\hline
\textit{Established:} Existence & Coupled scalar-vector system admits static spherical solutions (Sec.~\ref{sec:spherical}) \\
\textit{Conditional:} Scaling & Deep-MOND $\sqrt{g_{\mathrm{bar}} a_0}$ attractor via Chameleon feedback (Sec.~\ref{sec:bvp_results}) \\
\textit{Established:} $a_0$ & $a_0 = cH_0/(2\pi)$ as consistency relation ($0.66\sigma$ from observed value) \\
\hline
\textit{Plausibilized:} RAR shape & BVP extraction yields $\mu_{\mathrm{CRM}}(x) = [1-\exp(-x^\alpha)]^{-1}$, $\alpha = 0.310$ \\
\textit{Plausibilized:} SPARC & $\chi^2/\mathrm{dof} = 9.67$ using McGaugh proxy \\
\hline
\textit{Open:} Exact $\mu(x)$ & Analytical closed-form interpolation function from field equations \\
\textit{Open:} Full SPARC & Reanalysis with CRM-native $\mu_{\mathrm{CRM}}$ instead of McGaugh proxy \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
\item \textbf{Run~B:} CRM-fixed ($a_0 = cH_0/(2\pi)$, fixed). If Run~B achieves $\chi^2$ values comparable to Run~A (within, say, $\Delta\chi^2 \lesssim 2N_\mathrm{gal}$), this supports the consistency of the CRM-deduced value with galactic data---but does \emph{not} constitute an independent prediction, since $a_0 = cH_0/(2\pi)$ is constructed to reproduce the observed $a_0$ (Sec.~\ref{sec:a0_derivation}). Significantly worse $\chi^2$ would falsify this consistency relation.
\end{itemize}

\textit{Clarification and caveat on nomenclature:} ``Zero-parameter'' refers to the absence of free dark matter halo parameters or MOND acceleration parameters. The per-galaxy $\Upsilon_*$ values (one per galaxy, 171 galaxies = 171 additional parameters) are astrophysical nuisance parameters present in \textit{any} rotation curve analysis (including $\Lambda$CDM with NFW halos). However, readers should be aware that this label is potentially misleading: Run~A has $171 + 1$ free parameters ($\Upsilon_*$ per galaxy plus $a_0$), while Run~B has $171$ free parameters. The comparison tests only whether fixing $a_0$ to the CRM-predicted value degrades the fit, not whether the CRM is a genuinely parameter-free theory.
...
The elevated $\chi^2/\mathrm{dof}$ values reflect the fact that we use the empirical McGaugh interpolation~\eqref{eq:rar} rather than the CRM-native interpolation function (which remains to be extracted from the nonlinear BVP). The McGaugh function is known to provide only an approximate description of the full SPARC scatter \cite{Li2018}. The $\Delta\chi^2$ between Run~B (CRM-fixed) and Run~C (best global) is $1183$.

\textit{Critical assessment of fit quality:} The factor-of-$\sim 2$ increase in $\chi^2/\mathrm{dof}$ from Run~A ($4.88$) to Run~B ($9.67$) is statistically substantial and cannot be dismissed as a ``modest penalty.'' Taken at face value, the data disfavor the CRM-fixed value $a_0 = cH_0/(2\pi) = 1.042 \times 10^{-10}$\,m/s$^2$ over both the per-galaxy-free value (Run~A median: $0.733$) and the best global constant (Run~C: $0.880$). This is a genuine tension, not resolved by noting that Runs~B and~C share the same $N_\mathrm{dof}$. The CRM-deduced $a_0$ lies $\approx 15\%$ above the best-fit global value; at SPARC's precision this is a statistically significant discrepancy. The full interpretation depends on three open issues: (i)~the CRM-native interpolation function (unavailable until the BVP is solved analytically), (ii)~the value of the saturation factor $\mathcal{B}_0$ (not yet constrained independently), and (iii)~the possible systematic offset induced by using the McGaugh form for a model that predicts a different transition shape. The SPARC test as reported here is therefore an \emph{exploratory consistency check}, not a confirmed validation.
...
Items marked \dag{} are \emph{consistency relations} (the model is constructed to reproduce the observed value), not parameter-free predictions; see Sec.~\ref{sec:a0_derivation}, Epistemic caveat.}
\begin{ruledtabular}
\begin{tabular}{llll}
Item & Type & Test & Status \\
\hline
$a_0 = cH_0/(2\pi)$\dag{} & Consistency relation & SPARC MCMC & This paper \\
Flat rotation curves & Prediction & SPARC & This paper \\
Baryonic Tully-Fisher & Prediction & BTFR data & This paper \\
RAR universality & Prediction & SPARC & This paper \\
No DM halos w/o baryons & Prediction & LSST, SKA & Future \\
MOND weaker at high $z$ & Optional prediction & JWST, ELT & Future \\
$\alpha_T = 0$ (preserved) & Consistency check & GW170817 & Proven (Sec.~\ref{sec:alpha_T}) \\
$\rho_A = 0$ on FLRW & Consistency check & Cosmological fits & Proven (Sec.~\ref{sec:rho_A}) \\
Parasitic screening & Consistency check & Solar System & Proven (Sec.~\ref{sec:parasitic}) \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
A detailed comparison of the perturbation spectra (CMB $C_\ell$ and matter $P(k)$) between CRM and AeST would be highly informative but lies beyond the scope of this paper. Recent work has demonstrated that relativistic MOND theories with timelike vector fields -- including AeST -- can be reformulated as mimetic gravity theories, exposing deep conformal and disformal symmetries in the metric structure~\cite{DomenechGanz2025}. Whether the CRM vector sector admits a similar mimetic reformulation, potentially simplifying its stability analysis, is an open question.

\subsection{Independent convergence: Imprint-Fading theory}

Independently and by a different method, Kovacs~\cite{Kovacs2026} derives the same fundamental scale $a_0 = cH_0/(2\pi)$ from the equality of the Gibbons--Hawking temperature of de~Sitter spacetime and the Unruh temperature at acceleration~$a_0$. His ``Imprint-Fading'' (IF) theory introduces a scalar curvature-occupation field $s(\mathbf{x},t) \in (0,1)$ governed by a single dissipative evolution equation $\dot{s} = H_0[y - s^2/(1-s)]$, where $y \equiv g_\mathrm{N}/a_0$. In steady state, this yields the MOND simple interpolation function $\nu(y) = [1 + \sqrt{1+4/y}]/2$ \emph{exactly and algebraically} -- a result that is not fitted but follows from the fixed-point condition. Global asymptotic stability is proved via a Lyapunov functional.

The convergence of two independent frameworks on the same $a_0$ relation -- derived here from the saturation ODE attractor and Fourier frequency conversion (Sec.~\ref{sec:a0_derivation}), and by Kovacs from thermodynamic temperature equality -- strengthens the case for its physical significance. Three further points of comparison deserve emphasis:

\begin{enumerate}
\item \textbf{Complementary MOND derivations:} The IF theory obtains MOND as an exact algebraic equilibrium of a one-field dissipative equation; the CRM obtains it as an emergent numerical attractor of the two-field (scalar + vector) system via BVP extraction. Both routes converge on MOND phenomenology but from distinct mathematical structures.

\item \textbf{Non-equilibrium predictions:} The IF theory provides a quantitative relaxation spectrum ($\tau = 0.09$--$204$\,Gyr) and predicts hyperbolic halo decay after baryon stripping, with a sharp dynamical bifurcation at $s_0 = \ln 2$. These non-equilibrium predictions -- testable with JWST, Gaia~DR4, and WEAVE -- have no analogue in the CRM or in any prior MOND formulation.

\item \textbf{Dark energy equation of state:} The IF framework predicts $w \geq -1$ as a consequence of the null energy condition applied to the Fisher information metric on $s \in (0,1)$. This is directionally consistent with the DESI~DR2 preference for dynamical dark energy in the $w_0>-1$, $w_a<0$ quadrant~\cite{DESI2025} and with the CRM Lagrangian: the $f(R) = R + 2\gamma R^2$ sector of Paper~III generically yields $w \to -1$ from above, not from below. The phantom value $w = -1.355$ reported in Paper~I is an artefact of the phenomenological tanh fit to Pantheon+ data with diagonal covariance and should not be regarded as a prediction of the Lagrangian theory. We therefore regard $w \geq -1$ as the natural CRM prediction, in agreement with the IF result.
\end{enumerate}

The key difference in scope remains: the IF theory is currently developed at the non-relativistic galactic level (with a candidate covariant extension), while the CRM provides a complete cosmological framework validated against CMB, BAO, and supernova data. A synthesis of the IF thermodynamic structure with the CRM Lagrangian formulation may prove fruitful.
...
\textbf{Non-uniqueness of the vector sector.} We emphasize that the vector sector presented here is a \emph{minimal consistent realization} of the thermodynamic requirement, not a unique result. The thermodynamic argument (Sec.~\ref{sec:derivation}) establishes that an additional degree of freedom beyond the scalaron is \emph{necessary} and that it must be a timelike vector field (Sec.~\ref{sec:why_vector_not_scalar}). However, the specific coupling $\mathcal{F}$ (Eq.~\ref{eq:coupling}) is the simplest ansatz satisfying six constraints -- alternative forms with the same constraint compliance may exist. The non-uniqueness is threefold: (i)~the coupling function is constrained but not derived uniquely; (ii)~the $\mathrm{sech}^2$ modulation is motivated by P\"oschl-Teller consistency but could be replaced by other smooth decoupling functions; (iii)~the kinetic structure $F_{\mu\nu}F^{\mu\nu}$ is minimal but not the only ghost-free option (cf.\ generalized Proca theories). What \emph{is} unique is the existence and scaling behavior: a vector degree of freedom coupling to the scalaron gradient, suppressed in the Solar System, and producing $a_0 \sim cH_0$. The precise functional form awaits further theoretical development.

\begin{remark}[Status of galactic field equations]
The CRM field equations at the galactic scale have the following status:
\begin{itemize}
\item \textbf{Established:} Existence of the MOND-scale attractor and $\sqrt{g_N}$ scaling from the saturation ODE.
\item \textbf{Plausible:} The specific functional form of the interpolation function $\mu(x)$ from BVP extraction.
\item \textbf{Open:} Exact derivation of $\mu(x)$ from first principles without numerical fitting.
\end{itemize}
\end{remark}
...
The ontological picture is: two axioms (null-space fluctuation + thermodynamic optimization) $\to$ two degrees of freedom (scalar + vector) $\to$ three phenomena (dark energy, cosmological dark matter effects, MOND-compatible galactic dynamics) $\to$ one termination condition (standard physics below galactic scales). No $\Lambda$, no dark matter particles -- only geometry. The termination condition, however, is now the weakest link rather than a settled corner: it rests on a screening mechanism that the quadratic $f(R)$ sector does not deliver (Sec.~\ref{sec:scalar_screening_status}), so the return to standard physics at small scales is assumed rather than derived. The numerical BVP confirms that Chameleon feedback can produce a MOND-like attractor (median slope~$\approx 0.58$ across six decades of galaxy mass), while the exploratory 171-galaxy SPARC check also exposes a real tension: the CRM-fixed value $a_0=cH_0/(2\pi)$ remains within the broad empirical $a_0$ uncertainty but fits substantially worse than free or best-global $a_0$ under the McGaugh proxy. The extraction of the emergent interpolation function $\mu_{\mathrm{CRM}}(x) = [1 - \exp(-x^\alpha)]^{-1}$ with $\alpha = 0.310 \pm 0.001$ from the BVP (Eq.~\ref{eq:mu_crm}) reveals that the CRM-native transition is shallower than the McGaugh form ($\alpha = 0.5$) and provides an $11\times$ better fit to the numerical data. A full SPARC reanalysis using $\mu_{\mathrm{CRM}}$ is therefore required before the galactic sector can be regarded as validated. Semi-analytical EFT estimates confirm that vector perturbations contribute $\Delta C_\ell/C_\ell < 10^{-11}\%$ to the CMB, establishing Planck-level safety of the vector sector.
...
Companion paper. \url{https://github.com/research-line/crm-cosmology}.
